...
def init_model(model_name, model_config, data_config, emb_type, args=None, num_stu=None, mode="train", train_start=True):
    if model_name == "dkt":
        ...
        if emb_type.find("pt") == -1:
            model = GPT4KT(data_config["num_c"], data_config["num_q"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"]).to(device).to(args.local_rank)
        else:
            model = GPT4KT(data_config["num_c"], data_config["num_q"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], num_sgap=data_config["num_sgap"]).to(device).to(args.local_rank)
        if mode == "train" and train_start:
